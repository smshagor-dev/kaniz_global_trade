...
export default function SupplierInsurancePage() {
  const [form, setForm] = useState({ partnerId: '', providerName: 'Allianz Trade', policyType: 'CARGO_INSURANCE', insuredAmount: 10000, premiumAmount: 120, currencyCode: 'USD', coverageSummary: '', claimInstructions: '' })
  ...
  async function submit() {
    ...
    setForm({ partnerId: defaultPartner?.id || '', providerName: defaultPartner?.name || 'Allianz Trade', policyType: 'CARGO_INSURANCE', insuredAmount: 10000, premiumAmount: 120, currencyCode: 'USD', coverageSummary: '', claimInstructions: '' })
    refetch()
  }
  ...
  useEffect(() => {
    ...
    if (defaultPartner && !form.partnerId) {
      setForm((current) => ({ ...current, partnerId: defaultPartner.id, providerName: defaultPartner.name }))
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5 grid md:grid-cols-2 gap-3">
        <select value={form.partnerId} onChange={(e) => setForm((v) => ({ ...v, partnerId: e.target.value, providerName: partnerCatalog.find((item) => item.id === e.target.value)?.name || '' }))} className="border border-gray-200 rounded-lg px-3 py-2 text-sm">
